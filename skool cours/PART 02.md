# Docker - TP2 : Construction d'images, Registres et docker-compose
> **Objectifs du TP** :
...
```sh
dev $ docker image pull alpine:latest
...
dev $ docker image pull alpine:3.13.2
...
```
Docker télécharge plusieurs couches pour la dernière version l'image de base `alpine`.

Maintenant, découvrons les images téléchargées en relançant la commande `docker image ls`.
```sh
dev $ docker image ls
REPOSITORY   TAG       IMAGE ID       CREATED       SIZE
ubuntu       latest    4dd97cefde62   6 days ago    72.9MB
alpine       3.13.2    28f6e2705743   2 weeks ago   5.61MB
...
Docker utilise également des tags pour qualifier les images. Dans notre cas, les images avec les tags `3.13.2` et `latest` sont en réalité la **même image**. Par défaut, si on met juste `alpine`, Docker construit un nouveau conteneur avec la version marquée par un tag `latest`. C'est pour cette raison que ces deux images ont le même id, bien qu'il est sensé être unique.
...
Pour reprendre ce que nous avont vu avec la premiere prise en main de ngninx, téléchargeons une image officielle qui nous mache le travail.
```sh
dev $ docker image pull nginx
```
Puis:
```sh
dev $ docker container run -i -t --name=nginx nginx /bin/bash
```
Puis à l'intérieur du conteneur :
```
# nginx -version
```
La commande nous renvoie bien la version de nginx installée. Nous n'avons pas besoin de lancer apt-get ou un autre gestionnaire de paquet, nginx est déjà présent dans notre image.
...
Par défaut, Docker ne veut pas supprimer une image qui est liée à un conteneur existant que celui-ci soit arrêté ou non. Il faut donc d’abord supprimer le conteneur Nginx.
```sh
dev $ docker container rm nginx
```
On peut ensuite supprimer l’image :
```sh
dev $ docker image rm nginx
```
Observez la sortie : on remarque que chacune des couches du système de fichiers sont supprimées les unes après les autres.

## 6- Construction de notre propre image Nginx
...
- Grâce à la commande `docker container commit` qui va créer une nouvelle couche de système de fichiers avec les nouveaux fichiers d'un conteneur (par exemple on installe Nginx au dessus d'une image de base ubuntu).
...
## 7- Création d'une image à l'aide d'un Dockerfile
...
Veillez à quitter le conteneur dans lequel vous êtes. Puis préparez à travailler dans le répertoire de travail `HandsOn_03/beaugops-app/` (le répertoire est déjà présent). Nous allons être amenés à créer plusieurs fichiers dans ce répertoire :
...
Ce code représente une application qui utilise un framework Python qui s’appelle `Flask`. 

L’application est très basique : elle répond sur la route racine `/` par `Hello beaugops` et un code `HTTP 200`.
...
Maintenant que nous avons créé notre fichier Dockerfile, il est temps de créer notre fichier docker-compose qui va nous simplifier la construction et le lancement de notre application. Nous allons créer un fichier `docker-compose.yml` (toujours dans  `/HandsOn_03/beaugops-app/`) avec le contenu suivant :
```yaml
# beaugops-app/docker-compose.yml
---
version: '3'
services:
  app:
    build: .
    image: "registry_url/repository_name/beagopsapp:v0.1"
    ports:
    - "8000:8000"
```
...
Successfully tagged 261407191094.dkr.ecr.eu-west-1.amazonaws.com/duar/app:v0.1
```
...
```sh
dev $ docker image build -t=app:v0.1 .
```
...
On peut remarquer que l’ID de l’image construite nous est communiqué en fin de build : `92ad10b85282` ici.

On peut remonter dans les actions effectuées pour la construction de notre image avec la commande `docker image history`.
```
dev $ docker image history 92ad10b85282
...
```
Nous pouvons également utiliser l'utilitaire *dive* pour avoir une vue plus fine des actions et leurs résultats par couches via la commande: `dive 92ad10b85282`.

Nous pouvons maintenant voir la totalité des commandes lancées lors de l'exécution de la commande `docker image build`. Les instructions exécutées sont triées de la plus récente à la plus ancienne. Seule les 8 premières nous intéressent car ce sont les commandes produites par notre Dockerfile (les autres concernent les couches inférieures de l'image alpine). Chacune des instructions du Dockerfile produit bien une couche de fichiers puis une image avec un ID que Docker réutilise pour construire la couche suivante.
...
Creating network "beaugops-app_default" with the default driver
Creating beaugops-app_app_1 ... done
Attaching to beaugops-app_app_1
...
Hello beaugops!
```
On voit que l’application répond avec un status code HTTP 200 et le message “Hello beaugops!”.
## 8- Transférer notre image sur le registre

Maintenant que nous avons notre image locale, le but est de la transférer sur le **registre central** afin que tout le monde puisse en profiter en appelant la commande `docker-compose push`.

Pour ce faire il nous faut un registry prets à accueillir notre image. 
Si vous possédez un compte sur le DockerHub ou sur un registre privé, vous pouvez essayez d'effectuer vous même les étapes suivantes. Si ce n'est pas le cas, voici quand même ce qu'il se passe ;^) :
```
dev $ docker-compose push
Pushing app (registry_url/repository_name/beaugopsapp:v0.1)...
The push refers to a repository [registry_url/repository_name/beaugopsapp]
fe248af17b1b: Pushed
5a54a6dcff20: Pushed
50f589ab1c5f: Pushed
75fed4290ceb: Pushed
...
```sh
dev $ docker image push registry_url/repository_name/beaugopsapp:v0.1
```
En analysant la trace, on remarque que Docker détecte que nous voulons pousser notre image dans le dépôt **app** situé dans le registre accessible à l'adresse `registry_url/repository_name`.
